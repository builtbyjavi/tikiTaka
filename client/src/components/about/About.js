...
const About = () => {
    ...
    if (!loading) {
        video = images.filter((images) =>
            images.filename.replace(/_/g, "").toLowerCase().includes("video"))[0].url;
        banner = images.filter((images) =>
            images.filename.replace(/_/g, "").toLowerCase().includes("aboutbanner"))[0].url;
        headquarters = images.filter((images) =>
            images.filename.replace(/_/g, "").toLowerCase().includes("aboutheadquarters"))[0].url;
        header = images.filter((images) =>
            images.filename.replace(/_/g, "").toLowerCase().includes("aboutheader"))[0].url;
    }
    ...
    return (
        <React.Fragment>
                <div className="header-bg position-relative ">
                    <div className="position-absolute container top-0 end-0 w-75 pt-5 pe-0">
                    ...
                    <div className="text-wrapper position-absolute top-100 translate-middle-y text-black ps-4">
                        ...
                        <h1 className="display-1 fw-semibold m-0"> 
                            ...
                            And <br/> 
                            Innovation <br/> 
                            <span className={scroll ? "scroll" : 'invisible opacity-0'}>To <br/> 
                            Every <br/> 
                            Athlete <br/>
                            In The <br/>
                            World
                ...
                <div className="headquarters mt-1 position-relative">
                    ...
                    <div className="image-wrapper mw-100">
                        <img src={headquarters} alt="" className="image-fluid w-100" />
                    </div>
                    
                    <div className="p-5 position-absolute bottom-0 start-50 translate-middle-x w-100">
                    
                        <div className="row text-white text-bg mb-5">
